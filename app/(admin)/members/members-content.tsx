...
export default function MembersContent({ currentUserId }: Props) {
  ...
  const [newResetPasswordConfirm, setNewResetPasswordConfirm] = useState('')
  const [deletingId, setDeletingId] = useState<string | null>(null)

  const { data: members = [], isLoading } = useQuery({ queryKey: ['members'], queryFn: getMembers })

  const passwordError = newPassword && newPassword.length < 6 ? 'รหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร' : ''
  const passwordMismatch = newPasswordConfirm && newPassword !== newPasswordConfirm ? 'รหัสผ่านไม่ตรงกัน' : ''
  const resetPasswordError = newResetPassword && newResetPassword.length < 6 ? 'รหัสผ่านต้องมีอย่างน้อย 6 ตัวอักษร' : ''
  const resetPasswordMismatch = newResetPasswordConfirm && newResetPassword !== newResetPasswordConfirm ? 'รหัสผ่านไม่ตรงกัน' : ''
  ...
  return (
    <div className="space-y-6 w-full">
          ...
          <Card key={m.id}>
            <CardHeader>
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {m.id === currentUserId && <Badge variant="outline" className="text-xs">คุณ</Badge>}
                <div className="ml-auto flex gap-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="text-xs"
                    disabled={updateMutation.isPending || m.id === currentUserId}
                    title={m.id === currentUserId ? 'ไม่สามารถ Disable บัญชีตัวเองได้' : ''}
                  ...
                  {m.role !== 'superadmin' && (
                    <Button
                      ...
                      title={m.id === currentUserId ? 'ไม่สามารถลบบัญชีตัวเองได้' : ''}
        ...
        <DialogContent>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-1">
              ...
              />
              {passwordMismatch && <p className="text-xs text-red-500">{passwordMismatch}</p>}
            </div>
            <div className="space-y-1">
      ...
      <Dialog open={!!deleteDialog} onOpenChange={open => { if (!open) setDeleteDialog(null) }}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              disabled={deleteMutation.isPending || deleteDialog?.id === currentUserId}
      ...
      <Dialog open={!!resetDialog} onOpenChange={open => { if (!open) { setResetDialog(null); setNewResetPassword(''); setNewResetPasswordConfirm('') } }}>
        <DialogContent>
          <DialogHeader>
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1">
              ...
              <Input
                ...
                autoFocus
              />
              {resetPasswordError && <p className="text-xs text-red-500">{resetPasswordError}</p>}
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium">ยืนยันรหัสผ่านใหม่</label>
              <Input
                type="password"
                value={newResetPasswordConfirm}
                onChange={e => setNewResetPasswordConfirm(e.target.value)}
                placeholder="กรอกรหัสผ่านอีกครั้ง"
              />
              {resetPasswordMismatch && <p className="text-xs text-red-500">{resetPasswordMismatch}</p>}
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => { setResetDialog(null); setNewResetPassword(''); setNewResetPasswordConfirm('') }}>ยกเลิก</Button>
            <Button
              disabled={
                !newResetPassword.trim() ||
                newResetPassword.length < 6 ||
                newResetPassword !== newResetPasswordConfirm ||
